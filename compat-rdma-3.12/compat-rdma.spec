...
%define build_rxe %(if ( echo %{configure_options} | grep "with-rxe-mod" > /dev/null ); then echo -n '1'; else echo -n '0'; fi)
...
%{!?_release: %define _release 0.1.rxe}
...
%if %{build_rxe}
       echo >> %{RDMA_CONF_DIR}/openib.conf
       echo "# Load RXE modules" >> %{RDMA_CONF_DIR}/openib.conf
       echo "RXE_LOAD=no" >> %{RDMA_CONF_DIR}/openib.conf
%endif
